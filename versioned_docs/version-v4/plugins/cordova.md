...
在使用 Capacitor 开发应用时，可以同时使用 Cordova 插件。
...
查看项目中安装的 Capacitor 和 Cordova 插件（及其确切版本号）列表，请使用：
...
Capacitor 与某些 Cordova 插件之间可能存在兼容性问题。许多官方的 Cordova 插件不应使用，因为 Capacitor 提供了[官方替代方案](/plugins/official.md)。使用变量和钩子的 Cordova 插件可能部分兼容。一些 Cordova 插件完全不兼容（请参阅[此列表](/plugins/cordova.md#known-incompatible-plugins)）。

如果你发现现有 Cordova 插件存在问题，请通过提供问题的详细信息和插件信息[告知我们](https://github.com/ionic-team/capacitor/issues/new)。

### 变量与钩子

Capacitor 不支持 Cordova 的安装变量、自动配置或钩子，这是基于我们的理念：让你控制自己的原生项目源代码（这意味着钩子等功能是不必要的）。如果你的插件需要设置变量或配置，你需要通过将插件的 `plugin.xml` 与 iOS 和 Android 上的所需设置映射来手动应用这些配置设置。
...
如果已知某个插件会导致冲突或构建问题，则在运行 `npx cap sync` 时会跳过该插件。
...
- [`cordova-plugin-googlemaps`](https://github.com/mapsplugin/cordova-plugin-googlemaps)（在 iOS 上导致构建失败，仅针对 iOS 跳过）