...
首先，按照插件指南的[开始](/plugins.mdx)部分所述生成一个插件。
...
生成插件后，在 Android Studio 中右键单击 Java 插件类，从菜单中选择"Convert Java file to Kotlin file"选项。Android Studio 将引导您完成项目的 Kotlin 支持配置。完成后，再次右键单击 Java 类并重新选择转换选项，将其转换为 Kotlin 类。

## 插件基础

Capacitor 的 Android 插件是一个简单的 Java 类，继承自 `com.getcapacitor.Plugin`，并带有 `@CapacitorPlugin()` 注解。它包含一些带有 `@PluginMethod()` 注解的方法，供 JavaScript 调用。

生成插件后，您可以打开以生成时选择的插件类名命名的文件，开始编辑。
...
客户端可以发送任何可以 JSON 序列化的数据，例如数字、文本、布尔值、对象和数组。这些数据可以通过调用实例的 `getData` 字段访问，或者使用 `getString` 或 `getObject` 等便捷方法访问。
...
要失败或拒绝调用，请使用 `call.reject`，传入错误字符串，并可选择传入错误码和 `Exception` 实例：
...
在遵循本节之前，请确保您已设置好权限别名和状态接口。如果尚未设置，请参阅 [Web 指南中的权限部分](/plugins/creating-plugins/web-guide.md#权限)。
...
将每个 `@Permission` 中的权限字符串按插件功能的独立模块进行分组。如果您的插件在其他平台上需要权限但在 Android 上不需要，则使用相同的别名定义一个 `strings` 为空数组的权限。这将导致该权限别名的权限请求结果自动返回为"已授权"。
...
通过在 `@CapacitorPlugin` 注解中定义权限，`checkPermissions()` 和 `requestPermissions()` 方法应该可以完全正常工作。应用开发者可以根据需要手动请求权限。但是，最佳实践是同时将插件功能包装在自动权限请求中。

#### 权限回调

创建一个带有单个 `PluginCall` 参数的 void 方法，并使用 `@PermissionCallback` 注解，然后将该方法的名称作为字符串传递给权限请求调用。回调将在权限请求完成后运行。
...
将任何所需的[安装时权限](https://developer.android.com/guide/topics/permissions/overview#install-time)放在插件的 `AndroidManifest.xml` 中。不要添加运行时权限（会提示用户接受的权限）。这些应该由应用开发者添加到 Capacitor 应用的 manifest 中。请确保您的插件文档记录了应在应用中添加的任何必需运行时权限。
...
此错误可抛出以指示该功能目前无法使用，通常是因为它需要更新的 Android API 版本。
...
要在 Capacitor 屏幕之上展示原生屏幕，我们将使用 [Android 的 Intents](https://developer.android.com/guide/components/intents-filters)。Intents 允许您从应用或另一个应用启动 Activity。[查看常见 Intents](https://developer.android.com/guide/components/intents-common)

### 无返回结果的 Intents

大多数情况下，您只希望展示原生 Activity，这时您可以直接触发[相关操作](https://developer.android.com/guide/components/intents-common)。
...
有时当您启动一个 Intent 时，您期望返回一些结果。这种情况下，您需要使用 `startActivityForResult`。
