...
在 JavaScript 的 [`EventTarget`](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget) 对象（例如 `window` 或 `document`）上触发一个事件。如果可能，建议优先使用 [Plugin Events（插件事件）](/plugins/creating-plugins/android-guide.md#plugin-events)。
...
关于如何在环境之间处理传递数据的说明，可以[在此处找到](/main/reference/core-apis/data-types.md)。
