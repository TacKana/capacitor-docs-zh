...
为了使 Apple 和 Google 能够自动填充和保存凭据，必须在你的网站和应用之间配置双向关联。在本指南中，我们将遵循与[深度链接](deep-links#create-site-association-file-1)相同的步骤，但会增加 [Capacitor 配置](#set-capacitor-server-hostname)步骤和使用 `autocomplete` 属性。
...
按照 [Android 深度链接指南](deep-links#android-configuration)创建站点关联文件和相关的 `AndroidManifest.xml` 更改，并额外验证：
...
如果你目标是 Web，请遵循[深度链接指南](deep-links#website-configuration)。
