...
在应用程序中创建单元测试时，最佳实践是为被测单元的任何外部依赖项创建模拟对象。这包括你的组件或服务正在使用的 Capacitor 插件。

大多数模拟库通过获取一个对象并用 JavaScript 代理包装它来创建模拟，这样就可以检查对该对象方法的调用并控制方法的返回值。然而，Capacitor 插件在 JavaScript 层是作为代理实现的。不支持创建代理的代理，并且会失败。可以使用手动模拟来规避此问题。
...
Jest 测试框架内置了 <a href="https://jestjs.io/docs/manual-mocks" _target="blank">手动模拟</a> 功能。在项目根目录下创建一个 `__mocks__/@capacitor` 文件夹，Jest 将自动从该文件夹加载文件，而不是从 `node_modules` 加载。
...
Angular 项目（使用 Jasmine 作为测试框架的最常见场景）包含一个 `tsconfig.spec.json` 文件，该文件在执行单元测试时扩展了 `tsconfig.json` 基础配置。修改此文件以扩展你在基础级别可能拥有的任何 `paths` 映射。
...
然后更新你的 `tsconfig.spec.json` 文件以包含这些路径以及你可能希望用于单元测试的任何路径：
...
**注意：** `paths` 对象会被完全替换而不是合并，因此如果你在 `tsconfig.json` 中定义了任何路径，它们也**必须**包含在 `tsconfig.spec.json` 中。

## 模拟存根

有了手动模拟，现在就可以以所有常规方式编写测试来模拟和监视方法调用。
