...
Keyboard.addListener('keyboardWillShow', info => {
  console.log('键盘将要显示，高度：', info.keyboardHeight);
});
...
| 属性                       | 类型                                                      | 描述                                                                                                                                                                                                                                                                                                       | 默认值               | 自从   |
| -------------------------- | --------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ------ |
| **`resize`**               | <code><a href="#keyboardresize">KeyboardResize</a></code> | 配置键盘出现时应用的调整大小方式。仅在 iOS 上可用。                                                                                                                                                                                                                                                        | <code>native</code> | 1.0.0 |
| **`style**                 | <code><a href="#keyboardstyle">KeyboardStyle</a></code>   | 如果您的应用不支持深色/浅色主题变化，则覆盖键盘样式。如果未设置，键盘样式将取决于设备外观。仅在 iOS 上可用。                                                                                                                                                                                               |                     | 1.0.0 |
| **`resizeOnFullScreen`**   | <code>boolean</code>                                      | 存在一个 Android 错误，当应用处于全屏模式时（例如，如果使用 StatusBar 插件来覆盖状态栏），键盘无法调整 WebView 的大小。如果将此设置设为 true，则会添加一个解决方法，即使应用处于全屏模式也能调整 WebView 大小。仅适用于 Android。                                                                          |                     | 1.1.0 |
...
以下事件也支持使用 `window.addEventListener`：
...
| **`Native`**  | <code>'native'</code> | 整个原生 Web View 将在键盘显示/隐藏时调整大小。这会影响 `vh` 相对单位。                                        | 1.0.0 |
| **`None`**    | <code>'none'</code>   | 应用和 Web View 都不会调整大小。                                                                               | 1.0.0 |
