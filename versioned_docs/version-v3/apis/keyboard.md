...
| ------------------------ | --------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----- |
| **`resize`**             | <code><a href="#keyboardresize">KeyboardResize</a></code> | 配置键盘出现时应用调整大小的方式。仅在 iOS 上可用。                                                                                                                                                                                                                            | <code>native</code> | 1.0.0 |
| **`style`**              | <code>'dark' \| 'light'</code>                            | 如果您的应用不支持暗/亮主题切换，则覆盖键盘样式。如果未设置，键盘样式将取决于设备外观。仅在 iOS 上可用。                                                                                                                                     |                     | 1.0.0 |
| **`resizeOnFullScreen`** | <code>boolean</code>                                      | 存在一个 Android 错误，当应用处于全屏模式时（例如使用 StatusBar 插件覆盖状态栏），键盘无法调整 WebView 大小。如果此设置设为 true，则添加一种解决方法，即使在应用全屏时也能调整 WebView 大小。仅适用于 Android。 |                     | 1.1.0 |
...
的兼容性，以下事件也可与 `window.addEventListener` 一起使用：
