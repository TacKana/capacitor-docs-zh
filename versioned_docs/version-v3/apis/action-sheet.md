...
description: Action Sheet API 提供访问原生 Action Sheets（操作列表）的功能，这些列表从屏幕底部弹出并显示用户可执行的操作。
...
显示一个 Action Sheet 样式的模态框，其中包含各种供用户选择的选项。
...
| **`icon`**      | <code>string</code>                                                       | 选项的图标（ionicon 命名约定）。此选项仅在 Web 上支持。              | 1.0.0   |
