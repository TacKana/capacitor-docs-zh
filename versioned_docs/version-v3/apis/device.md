...
| 属性         | 类型                | 描述                                                                                                                                                                                                                                                              | 自版本 |
| ------------ | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| **`uuid`**   | <code>string</code> | 应用程序可用的设备 UUID。在现代移动平台上，此标识符可能会发生变化，因为这些平台只允许每个应用程序的安装 UUID。在 Web 平台上，会生成一个随机标识符并存储在 localStorage 中供后续调用使用。                                                                         | 1.0.0  |

#### DeviceInfo（设备信息） {#deviceinfo}
...
#### GetLanguageCodeResult（获取语言代码结果） {#getlanguagecoderesult}
