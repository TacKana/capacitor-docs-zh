...
> **先决条件：**在继续之前，请熟悉 <a href="https://capacitorjs.com/docs/v3/ios/custom-code" target="_blank">Capacitor 自定义原生 iOS 代码文档</a>。
...
iOS 并不直接提供"锁定"或"解锁"屏幕方向的机制。相反，它允许你以编程方式设置允许哪些方向。

要实现这一点，我们需要在 `AppDelegate.swift` 的 `AppDelegate` 类中添加一个方法：
...
注意该函数返回 `ScreenOrientationPlugin.supportedOrientations`。这个属性还不存在，所以让我们将它作为私有静态类成员添加到 `ScreenOrientationPlugin` 类中：

```swift
public static var supportedOrientations = UIInterfaceOrientationMask.all
```

通过设置上述代码，我们告诉 iOS 我们只想支持由 `ScreenOrientationPlugin.supportedOrientations` 值定义的方向。可以想象，`UIInterfaceOrientationMask.all` 枚举值支持所有方向。当我们编写锁定屏幕方向的代码时，将选择一个更具限制性的枚举值。
...
1. `completion: @escaping (UIInterfaceOrientationMask) -> Void` 告诉此方法的调用者，他们必须提供一个在方法执行完成时将被调用的函数，并且我们将通过 `completion(mask)` 向该函数传递一个 `UIInterfaceOrientationMask` 值。
2. `UIDevice.current.setValue(orientation, forKey: "orientation")` 为设备设置屏幕方向，但不会将屏幕旋转到该方向。
3. `UINavigationController.attemptRotationToDeviceOrientation()` 将尝试将应用旋转到前一行代码中设置的屏幕方向。
