...
Capacitor 提供原生 Android 运行时，使开发者能够在 JavaScript 与原生 Java 或 Kotlin 代码之间进行通信。
...
> 要使用 Android 模拟器，必须使用 API 24+ 的系统镜像。系统 WebView 在模拟器上不会自动更新。物理设备只要系统 WebView 已更新，最低可支持 API 21。
...
在 Android Studio 中，首先选择设备或模拟器，然后点击运行或调试按钮来启动应用。除非您正在调试 Java 或 Kotlin 代码，否则建议使用运行按钮。

![运行应用](../../../../static/img/v3/docs/android/running.png)

## 问题排查

如果您在入门时遇到任何问题，可以查阅 [Android 问题排查指南](/main/android/troubleshooting.md)。如果需要帮助，请随时 [发起讨论](https://github.com/ionic-team/capacitor/discussions/)。

## 下一步

如果您的应用已成功运行，现在可以继续开发和构建您的应用了。使用各种可用的 API、Capacitor 或 Cordova 插件，或自定义原生代码来构建应用的其余部分。

## 延伸阅读

请遵循以下 Android 特定指南，以获取有关为应用设置权限、更新依赖项、构建插件等的更多信息：
