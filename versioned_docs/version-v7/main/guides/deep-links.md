...
Universal Links（iOS）和 App Links（Android）能够将用户直接引导至原生应用内的特定内容（通常称为深度链接）。

当用户点击深度链接时，会直接进入你的应用，无需先经过设备浏览器或网站。如果应用未安装，用户会被引导至网站。如果用户直接访问网站，则会停留在网站上。这使得深度链接成为为 Web、iOS 和 Android 构建的跨平台应用的绝佳功能：提供无缝的移动体验，并优雅地回退到网站。

优势：

- **安全性高**：Universal/App Links 使用 HTTPS URL，链接到你拥有的网站域名，确保其他应用无法使用你的链接。
- **无缝体验**：一个 URL 同时适用于网站和应用，确保用户能够成功访问所需内容而不会出错。
- **提升参与度**：链接可以从电子邮件客户端、搜索引擎结果等位置打开。

## 演示视频

以下是实际效果示例。在这个例子中，用户已安装原生应用。他们点击电子邮件中的应用链接，直接进入应用本身。首先点击根链接（https://beerswift.app），将用户引导至应用主页面。然后点击深度链接（https://beerswift.app/tabs/tab3），将用户带到 Tab3 页面。
...
如果你的网站和应用路径不匹配，你需要实现更高级的 URL 模式匹配（有关示例，请参阅[本指南](https://devdactic.com/universal-links-ionic/)）。但如果你的移动应用和 Web 应用使用相同的代码库，这将非常简单——只需重定向到相同的 URL。以下示例假设这种情况。
...
React 有多种实现方式。一种方法是将 App API 监听器功能封装在新组件中，然后将其添加到 `App.tsx` 中。首先创建 `AppUrlListener.tsx`，然后导入 React Router 的 `useHistory` 钩子以及 Capacitor App API：
...
然后将其添加到 `IonReactRouter` 内部（或应用启动的任何位置，确保 History 钩子可用）：
...
VueJS 提供了一个与 Vue 原生集成的官方路由系统，称为 Vue Router。要使用 Vue Router 设置深度链接，请从配置所有路由的文件开始（通常是 `routes.js` 或类似文件）。

首先从插件导入 Capacitor 的 `App`，以及 `Vue` 和 `VueRouter`。
...
为了让 Apple 和 Google 允许深度链接打开你的应用，必须在你的网站和应用之间建立双向关联。必须为每个平台创建一个文件，并将其放置在网站上的 `.well-known` 文件夹中，例如：https://beerswift.app/.well-known/。
...
首先，登录 [Apple Developer 网站](https://developer.apple.com)。导航至 "Certificates, Identifiers, & Profiles" 部分并选择你的应用标识符。记下 Team ID 和 Bundle ID，然后在 Capabilities 下，开启 "Associated Domains" 并保存：
...
接下来，将此文件上传到你的网站（需托管在 HTTPS 上），然后使用 Apple 的工具 [此处](https://search.developer.apple.com/appsearch-validation-tool/) 验证其配置是否正确。URL 应遵循以下格式：https://beerswift.app/.well-known/apple-app-site-association

### 添加关联域名

最后一步是配置 iOS 应用以识别传入的链接。打开 Xcode，然后导航至 Signing & Capabilities。点击 "+ Capability"，然后选择 Associated Domains。在出现的 Domains 条目中，使用格式 `applinks:yourdomain.com` 进行编辑：

![Xcode 关联域名](/img/v6/docs/guides/deep-links/xcode-associated-domain.png)

## Android 配置 {#android-configuration}

Android 配置涉及创建站点关联文件，并配置原生应用使用 intent 过滤器来识别应用链接。
...
将文件部署到你的网站（需托管在 HTTPS 上），然后在 Asset Link 工具中点击 "Test statement" 按钮进行验证。如果配置正确，将显示成功消息：
...
将关联文件放在 `src/.well-known` 下。接下来，配置构建过程以完全按原样部署这些文件（确保 Apple/Google 能够正确读取它们）。打开 `angular.json`，在 `architect => assets` 下，向数组中添加一个新条目：
...
将关联文件放在 `static/.well-known` 下。无需额外步骤；只需构建并部署网站即可。

### React

将关联文件放在 `public/.well-known` 下。无需额外步骤；只需构建并部署网站即可。
