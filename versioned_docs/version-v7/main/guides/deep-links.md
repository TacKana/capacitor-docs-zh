...
当用户点击深度链接时，用户会直接进入你的应用，而无需先通过设备的 Web 浏览器或网站路由。如果应用未安装，则用户会被引导到网站。如果用户直接导航到网站，他们将停留在网站上。这使得深度链接成为为 Web、iOS 和 Android 构建的跨平台应用的优秀功能：无缝的移动体验，并优雅地回退到网站。

好处：

- **安全**：Universal/App Links 使用链接到你拥有的网站域名的 HTTPS URL，确保没有其他应用可以使用你的链接。
- **无缝体验**：同一个 URL 既适用于你的网站也适用于你的应用，确保用户可以成功访问他们正在寻找的内容而不会出错。
- **增加参与度**：链接可以从电子邮件客户端、搜索引擎结果等中打开。

## 演示视频

以下是实际效果。在此示例中，用户已安装原生应用。他们从电子邮件中点击应用链接，直接被带入应用本身。首先，点击根链接（https://beerswift.app），将用户引导到主应用页面。接下来，点击深度链接（https://beerswift.app/tabs/tab3）将用户带到 Tab3 页面。
...
如果你的网站和应用路径不匹配，你将需要实现更高级的 URL 模式匹配（参见[此指南](https://devdactic.com/universal-links-ionic/)中的示例）。但是，如果你的移动应用和 Web 应用使用相同的代码库，这非常简单——只需重定向到相同的 URL。以下示例假设这种情况。
...
接下来，定义 AppUrlListener 组件，监听 `appUrlOpen` 事件并在找到深度链接时重定向：
...
然后将其添加到 `IonReactRouter` 内部（或你的应用引导的任何地方，只需确保 History hook 可用）：
...
VueJS 提供了一个与 Vue 原生集成的第一方路由系统，称为 Vue Router。要使用 Vue Router 设置深度链接，首先在你用来配置所有路由的文件中开始（通常是 `routes.js` 或类似文件）。
...
接下来，使用 Vue Router 配置路由（更多信息请参见 [Vue Router 入门](https://router.vuejs.org/guide/#javascript)）。
...
最后，我们需要注册应用以进行深度链接。为此，我们在 Capacitor App 上为 `appUrlOpen` 事件添加一个事件监听器。Capacitor 将捕获该事件，然后我们将其传递给 Vue Router 以导航到请求的页面。
...
为了使 Apple 和 Google 允许深度链接打开你的应用，必须在你的网站和应用之间创建双向关联。必须为每个平台创建一个文件，并将其放置在你网站的 `.well-known` 文件夹中，如下所示：https://beerswift.app/.well-known/。
...
首先，登录 [Apple Developer 网站](https://developer.apple.com)。导航到"Certificates, Identifiers, & Profiles"部分，选择你的应用标识符。记下 Team ID 和 Bundle ID，并在 Capabilities 下，切换"Associated Domains"，然后保存：
...
接下来，将文件上传到你的网站（托管在 HTTPS 上），然后使用 Apple 的[工具](https://search.developer.apple.com/appsearch-validation-tool/)验证其配置是否正确。URL 应遵循以下格式：https://beerswift.app/.well-known/apple-app-site-association
...
Android 配置涉及创建站点关联文件并使用 Intent Filter 配置原生应用以识别应用链接。
...
```shell
keytool -genkey -v -keystore 密钥名称.keystore -alias 别名 -keyalg RSA -keysize 2048 -validity 10000
```
...
将文件部署到你的网站（托管在 HTTPS 上），然后通过点击 Asset Link 工具中的"Test statement"按钮进行验证。如果配置正确，将显示一条成功消息：
...
将关联文件放在 `src/.well-known` 下。接下来，配置构建过程以原样部署这些文件（确保 Apple/Google 可以正确读取它们）。打开 `angular.json`，在 `architect => assets` 下，向数组添加一个新条目：
...
要验证网站和原生应用配置是否正确，网站需要托管站点关联文件，但应用不需要在应用商店中。
