...
首先在终端中打开你的项目，然后按照[将 Capacitor 添加到 Web 应用](/main/getting-started/installation.md#将-capacitor-添加到你的-web-应用)或[将 Capacitor 添加到 Ionic 应用](/main/getting-started/with-ionic.md#向现有的-ionic-项目安装-capacitor)的指南进行操作。
...
项目根目录下会创建 android 和 ios 文件夹。这些是完全独立的原生项目工件，应被视为你应用的一部分（即，将其检入源代码管理，在它们自己的 IDE 中编辑等）。此外，在 `package.json` 的 `dependencies` 中找到的任何 Cordova 插件都会被 Capacitor 自动安装到每个新的原生项目中（排除任何[不兼容的插件](/plugins/cordova.md#已知不兼容的插件)）：
...
请注意，任何[不兼容或导致构建问题的插件](/plugins/cordova.md#已知不兼容的插件)都会自动跳过。
