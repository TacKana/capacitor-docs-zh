...
可能并非所有功能都有现成的 [Capacitor 插件](/plugins.mdx)——这没关系！你完全可以直接在你的应用中编写 WebView 可访问的原生代码。
...
首先，通过 [打开 Xcode](/main/ios/index.md#opening-the-ios-project) 创建 `EchoPlugin.swift` 文件：右键点击 **App** 组（在 **App** 目标下），从上下文菜单中选择 **New File...**，在窗口中选择 **Swift File**，然后创建文件。
...
> 必须使用 `@objc` 装饰器，以确保 Capacitor 运行时（它必须使用 Objective-C 来支持动态插件）能够识别它。
...
在 JavaScript 中，我们使用 `@capacitor/core` 中的 `registerPlugin()` 来创建一个与我们的 Swift 插件关联的对象。
...
> `registerPlugin()` 的第一个参数是插件名称，它必须与 `EchoPlugin.swift` 中的 `jsName` 匹配。

**TypeScript**

我们可以通过定义一个接口，并在调用 `registerPlugin()` 时使用它，来为这个关联的对象定义类型。
...
`registerPlugin()` 的泛型参数定义了关联对象的结构。如果需要，你可以使用 `registerPlugin<any>('Echo')` 来忽略类型。我们不做评判。❤️
