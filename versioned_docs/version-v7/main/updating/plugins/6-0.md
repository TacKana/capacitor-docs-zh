---
title: 将插件更新到 6.0
description: 将 Capacitor 从早期版本更新到 v6 的插件指南
translated: true
...
如果你的插件在 `.m` 文件中有 `CAP_PLUGIN_METHOD(removeAllListeners, CAPPluginReturnPromise)`，现在可以移除它，该方法现在对所有插件都可用，无需显式定义。
...
`addListener` 签名已更改为只返回一个 `Promise`，移除 `& PluginListenerHandle`。
