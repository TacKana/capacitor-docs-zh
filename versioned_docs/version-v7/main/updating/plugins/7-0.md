---
title: 将插件更新到 7.0
description: 将 Capacitor 从 v6 更新到 v7 的插件指南
translated: true
...
一些已弃用的插件定义和方法，如 `registerWebPlugin`，已被移除，请查看 Capacitor 3 插件升级指南获取更多信息。

Capacitor 的辅助属性 `platform` 和 `isNative` 已被移除，请改用 `getPlatform()` 和 `isNativePlatform()` 方法。

## Android

`BridgeFragment` 类已被移除，如果插件使用该类来呈现 `Fragment`，则需要创建自己的版本。
...
最后，你可以移除 `.prettierignore` 中已经在 `.gitignore` 中的条目，因为从 Prettier 3.0.0 开始，它会默认忽略 `.gitignore` 文件中的文件。
如果 `.prettierignore` 中的所有条目都已经在 `.gitignore` 中，你可以完全删除 `.prettierignore` 文件。

#### 更新 rollup

将 `rollup` 更新到 `^4.30.1`。然后将 `rollup.config.js` 文件重命名为 `rollup.config.mjs`。
最后，更新 build 脚本以使用新的文件扩展名：
...
### 更新 targetSDK / compileSDK 到 35 并将 minSDK 更新到 23
...
在兼容 SPM 的插件中，更新 `Package.swift` 文件以使用固定版本而非主分支。
