...
锁定屏幕方向仅对 Capacitor 的 View Controller 生效，不会影响其他已呈现的 View Controller（例如 Browser 插件所呈现的控制器）。
如需同时锁定已呈现的 View Controller，可以将以下代码添加到应用的 `AppDelegate.swift` 文件中：
...
默认情况下，iPad 允许多任务处理，且其方向无法被锁定。若需在 iPad 上锁定方向，请将 `Requires Full Screen` 选项设为 `YES`，即在 `Info.plist` 中添加以下配置：
