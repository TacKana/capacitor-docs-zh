...
Action Sheet API 提供对原生操作表（Action Sheets）的访问，操作表从屏幕底部弹出，显示用户可以执行的操作。
...
本插件将使用以下项目变量（在您应用的 `variables.gradle` 文件中定义）：
...
| **`icon`**    | <code>string</code>                                                       | 选项的图标（遵循 ionicon 命名规范）。此选项仅在 Web 平台上受支持。     | 1.0.0  |
...
| **`Destructive`**  | <code>'DESTRUCTIVE'</code>  | 用于表示破坏性操作的选项样式。                                                                   | 1.0.0  |
| **`Cancel`**       | <code>'CANCEL'</code>       | 用于取消操作表的选项样式。如果使用，应将其应用于最后一个可用选项。                               | 1.0.0  |
