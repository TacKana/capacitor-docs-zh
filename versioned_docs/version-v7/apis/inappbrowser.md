...
description: InAppBrowser 插件提供了一个网页浏览器视图，允许你在外部加载任何网页。它的行为类似于标准网页浏览器，可用于加载不受信任的内容而不会危及应用程序的安全性。它提供三种不同的 URL 打开方式：在 WebView 中、在应用内系统浏览器（Android 使用 Custom Tabs，iOS 使用 SFSafariViewController）以及在设备默认浏览器中。
...
在你的移动应用中使用应用程序内的自定义 web view 打开给定 URL 的网页内容。
...
在你的移动应用中打开给定 URL 的网页内容，iOS 使用 SafariViewController，Android 使用 Custom Tabs。
...
在单独的浏览器中打开给定 URL 的网页内容，该浏览器位于你的移动应用程序外部。
...
| 属性                  | 类型                                                      | 描述                                                          |
| --------------------- | --------------------------------------------------------- | ------------------------------------------------------------- |
| **`options`**         | <code><a href="#webviewoptions">WebViewOptions</a></code> | 包含要应用于 Web View 的一些配置项的结构体。                  |
| **`customHeaders`**   | <code>{ [key: string]: string; }</code>                   | 一个要与请求一起发送的自定义头部信息的映射。                  |
...
| **`customWebViewUserAgent`**          | <code>string \| null</code>                                             | 设置打开 Web View 时使用的自定义用户代理。如果为空或未设置，该参数将被忽略。                     |
...
| **`allowInLineMediaPlayback`**            | <code>boolean</code>                                  | 允许 HTML5 媒体内联播放，在浏览器窗口内显示，而非使用设备特定的播放界面。注意：HTML 的 video 元素也必须包含 webkit-playsinline 属性。                                                                       |
...
| **`allowsBackForwardNavigationGestures`** | <code>boolean</code>                                  | 在 Web View 中启用前进和后退滑动手势。                                                                                                                                                                     |
...
| 属性          | 类型                                                                  | 描述                                                             |
| ------------- | --------------------------------------------------------------------- | ---------------------------------------------------------------- |
| **`options`** | <code><a href="#systembrowseroptions">SystemBrowserOptions</a></code> | 一个包含要应用于系统浏览器的配置项的结构体。                         |
...
| 属性          | 类型                                                                                | 描述                                   |
| ------------- | ----------------------------------------------------------------------------------- | -------------------------------------- |
| **`android`** | <code><a href="#androidsystembrowseroptions">AndroidSystemBrowserOptions</a></code> | Android 平台特有的系统浏览器选项。      |
| **`iOS`**     | <code><a href="#iossystembrowseroptions">iOSSystemBrowserOptions</a></code>         | iOS 平台特有的系统浏览器选项。          |
...
| **`bottomSheetOptions`**  | <code><a href="#androidbottomsheet">AndroidBottomSheet</a></code> | 当 `viewStyle` 选择为底部表单（BOTTOM_SHEET）时，设置其选项。如果 `viewStyle` 是 FULL_SCREEN，则此设置将被忽略。                       |
...
| **`enableBarsCollapsing`** | <code>boolean</code>                                    | 启用向下滚动时栏折叠。                 |
