...
description: Preferences API 提供了一个简单的键值对持久化存储，用于存储轻量级数据。
...
**注意**：此 API **不**适合用作本地数据库。如果你的应用需要存储大量数据、具有高读写负载或需要复杂查询，我们建议你考虑基于 SQLite 的解决方案。其中一个解决方案是 [Ionic Secure Storage](https://ionic.io/docs/secure-storage)，这是一个支持完全加密的基于 SQLite 的引擎。[Capacitor 社区](https://github.com/capacitor-community/) 也构建了许多其他存储引擎。
...
| 属性           | 类型                | 说明                                                                                                                                                                                                                                                                                                                             | 默认值                        | 始于 |
| -------------- | ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------- | ---- |
| **`group`**    | <code>string</code> | 设置偏好设置组。偏好设置组用于组织键/值对。使用值 'NativeStorage' 可向后兼容 [`cordova-plugin-nativestorage`](https://www.npmjs.com/package/cordova-plugin-nativestorage)。警告：使用 'NativeStorage' 组时，`clear()` 方法可能会删除意外值。                                                                                     | <code>CapacitorStorage</code> | 1.0.0 |
...
| 属性           | 类型                        | 说明                                                                                                                               | 始于 |
| -------------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ---- |
| **`value`**    | <code>string \| null</code> | 与给定键关联的偏好设置值。如果之前未设置值或值已被移除，则 `value` 将为 `null`。                                                   | 1.0.0 |
...
| **`value`**    | <code>string</code> | 要用关联键在偏好设置中设置的值。               | 1.0.0 |
...
| **`existing`**     | <code>string[]</code> | 已迁移或以其他方式存在于偏好设置中且在 Capacitor 2 Preferences 插件中具有值的键数组。                                 | 1.0.0 |
