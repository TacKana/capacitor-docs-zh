...
Capacitor 与某些 Cordova 插件之间可能存在兼容性问题。许多官方的 Cordova 插件不应使用，因为 Capacitor 提供了 [官方替代方案](/plugins/official.md)。使用变量和钩子的 Cordova 插件可能部分兼容。有些 Cordova 插件则完全不兼容（请参阅 [此列表](/plugins/cordova.md#known-incompatible-plugins)）。
