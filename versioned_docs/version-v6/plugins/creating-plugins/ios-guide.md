...
为 iOS 构建 Capacitor 插件涉及编写 Swift（或 Objective-C）来与苹果的 iOS SDK 交互。

## 入门指南

首先，按照插件指南的[入门](/plugins/creating-plugins/overview.md)部分所述生成一个插件。
...
Capacitor 的 iOS 插件有两个简单的 Swift 类，一个是实现类，继承 `NSObject`，您应该在其中放置插件逻辑；另一个继承 `CAPPlugin` 和 `CAPBridgedPlugin`，包含一些可以从 JavaScript 调用的导出方法，并包装实现类的方法。
...
每个插件方法都会收到一个 `CAPPluginCall` 实例，其中包含来自客户端的所有插件方法调用信息。

客户端可以发送任何可以 JSON 序列化的数据，例如数字、文本、布尔值、对象和数组。这些数据可以通过调用实例的 `options` 字段访问，或者使用便利方法如 `getString` 或 `getObject` 访问。传递和访问某些值有一些需要注意的特殊情况，详见[单独讨论](/main/reference/core-apis/data-types.md#ios)。
...
注意在 `CAPPluginCall` 实例上访问数据的各种方式，包括如何使用 `guard` 强制要求选项。
...
要使调用失败或拒绝调用，请调用 `reject()`，传入一个错误字符串，并可选地传入错误代码和 `Error` 实例：
...
为了确保 Capacitor 能够识别您的插件，插件生成器做了两件事：将您的 Swift 类导出到 Objective-C，并注册插件方法。
...
这使得 `echo` 方法可用于 Capacitor Web 运行时，向 Capacitor 指示 echo 方法将返回一个 Promise。

要向插件添加更多方法，请在 `.swift` 插件类中使用 `@objc` 关键字创建它们，并在 `pluginMethods` 数组中添加一个新的 `CAPPluginMethod` 条目。

## 权限

如果您的插件在 iOS 上需要最终用户的权限，那么您需要实现权限模式。

在遵循本节之前，请确保您已设置了权限别名和状态接口。如果尚未设置，请参阅 [Web 指南中的权限部分](/plugins/creating-plugins/web-guide.md#permissions)。
...
此方法应返回插件中权限的当前状态，应为一个与您定义的[权限状态定义](/plugins/creating-plugins/web-guide.md#permission-status-definitions)结构匹配的字典。通常，这些信息可以直接从您使用的框架中获得。
...
如果框架支持基于块的 API 来请求权限，则可以在单个方法内完成该操作。

在下面的示例中，我们从 `AVCaptureDevice` 请求视频访问权限，然后使用我们自己的 `checkPermissions` 方法检查当前的权限状态，最后完成调用。
...
如果框架使用委托（或回调）API，完成操作意味着需要保存原始调用，然后在回调被调用后检索它。
...
当需要多种类型的权限时，[DispatchGroup](https://developer.apple.com/documentation/dispatch/dispatchgroup) 是一种方便的方式来同步多个调用。
...
此错误可被抛出以指示该功能当前无法使用，通常是因为它需要更新的 iOS 版本。
...
您可以通过使用 [Capacitor 的 `UIViewController`](/main/reference/core-apis/ios.md#viewcontroller) 在应用之上呈现原生界面。
...
Capacitor iOS 插件既是 CocoaPods 库也是 Swift Package Manager 库，因此要添加依赖、所需的框架或任何其他高级配置，您必须编辑 CocoaPods 的 `.podspec` 和 SPM 的 `Package.swift`。这些文件由插件生成器创建。
请查阅 [podspec 参考](https://guides.cocoapods.org/syntax/podspec.html) 了解 CocoaPods 的所有可用选项。
请查阅 [Package Description](https://docs.swift.org/package-manager/PackageDescription/PackageDescription.html) 了解 SPM 的所有可用选项。

为添加 `FirebaseFirestore` 版本 `11.8.0` 或更高但低于 `12.0.0` 的依赖项，示例 `.podspec` 配置如下：
...
为添加 `FirebaseFirestore` 版本 `11.8.0` 或更高但低于 `12.0.0` 的依赖项，示例 `Package.swift` 配置如下：
