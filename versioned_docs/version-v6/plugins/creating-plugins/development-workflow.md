...
创建好新插件后，你就可以开始在各种平台上实现功能了。

## 实现新方法

要在你的插件中实现新功能，首先需要在 `src/definitions.ts` 文件中为你插件的导出 TypeScript 接口定义方法签名。

在下面的示例中，我们添加了 `openMap()` 方法，它接收 `latitude` 和 `longitude` 参数。最佳实践是为方法参数定义接口，这样应用中可以导入和使用这些接口。
...
要在开发过程中本地测试插件，可以使用 `npm install` 将插件文件夹链接到你的应用中，并指定插件路径。
...
要从你的应用中取消链接本地插件，使用 `npm uninstall` 加上你的插件包名。
...
插件模板内置了 [`@capacitor/docgen`](https://github.com/ionic-team/capacitor-docgen)，它会将生成的文档写入 `README.md`。文档会在 `npm run build` 期间生成。你也可以手动运行：
...
现在你的包可以在任何 Capacitor 应用中使用 `npm install echo` 安装了。