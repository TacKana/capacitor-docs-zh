...
大多数模拟库通过获取一个对象并将其包装在 JavaScript 代理中来创建模拟，这样就能检查对该对象方法的调用并控制方法的返回值。然而，Capacitor 插件在 JavaScript 层是作为代理实现的。不支持创建代理的代理，这会导致失败。手动模拟可以用来规避这个问题。

## 手动模拟

手动模拟允许用户轻松地存根整个 JavaScript 模块的功能。因此，当测试执行 `import { Storage } from '@capacitor/storage'` 时，不会加载真正的 `Storage` JavaScript 代理对象，而是会加载类似这样的内容：
...
由于这是一个纯 JavaScript 对象而非代理对象，因此很容易进行监视。同时，由于这是一个模拟对象，它不会尝试进行任何原生调用。这使得手动模拟成为测试使用 Capacitor 插件的代码时的理想选择。

### Jest

Jest 测试框架内置了 <a href="https://jestjs.io/docs/manual-mocks" _target="blank">手动模拟</a> 功能。在项目的根目录下创建 `__mocks__/@capacitor` 文件夹，Jest 将自动从该文件夹加载文件，而不是从 `node_modules` 加载。
...
Angular 项目（使用 Jasmine 作为测试框架的最常见场景）包含一个 `tsconfig.spec.json` 文件，在执行单元测试时扩展 `tsconfig.json` 基本配置。修改此文件以扩展基本级别可能存在的任何 `paths` 映射。
...
**注意：** `paths` 对象会被完全替换而不是合并，因此如果你在 `tsconfig.json` 中定义了任何路径，它们也 _必须_ 包含在 `tsconfig.spec.json` 中。

## 模拟存根

有了手动模拟之后，现在就可以以所有常用方式编写测试来模拟和监视方法调用。
