...
为了使 Apple 和 Google 能够自动填充和保存凭据，必须配置您的网站和应用之间的双向关联。在本指南中，我们将遵循与[深度链接](deep-links#创建站点关联文件)相同的步骤，但会增加 [Capacitor 配置](#设置-capacitor-server-hostname)的步骤以及 `autocomplete` 属性的使用。
...
按照 [Android Deep Links 指南](deep-links#android-配置)创建站点关联文件和相关的 `AndroidManifest.xml` 更改，并额外验证：
...
如果您目标是 Web，请按照[深度链接指南](deep-links#网站配置)进行操作。
