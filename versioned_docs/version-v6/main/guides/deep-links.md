...
当点击深度链接后打开原生应用时，移动操作系统不会自动知道将用户路由到哪里。这必须在应用本身中使用 Capacitor [App API](/apis/app.md) 在应用启动时实现。

如果您的网站和应用路径不匹配，您将需要实现更高级的 URL 模式匹配（参见[此指南](https://devdactic.com/universal-links-ionic/)的示例）。但如果您的移动应用和 Web 应用使用相同的代码库，这非常简单 - 只需重定向到相同的 URL。以下示例假设如此。
...
然后将其添加到 `IonReactRouter` 内部（或您的应用启动的任何地方，只要确保 History hook 可用即可）：
...
VueJS 提供了一个名为 Vue Router 的第一方路由系统，它与 Vue 原生集成。要使用 Vue Router 设置深度链接，请从您用于配置所有路由的文件开始（通常是 `routes.js` 或类似文件）。
...
建议使用 `mode: history`，这样您就不需要处理 `#` 了。

让 Vue 知道您正在使用 Vue Router，并在 Vue 中注册路由：
...
最后，我们需要注册我们的应用以支持深度链接。为此，我们在 Capacitor App 上为 `appUrlOpen` 事件添加一个事件监听器。Capacitor 将捕获此事件，然后我们将其交给 Vue Router 来导航到请求的页面。
...
为了使 Apple 和 Google 允许深度链接打开您的应用，必须在您的网站和应用之间创建双向关联。必须为每个平台创建一个文件，并放置在您网站上的 `.well-known` 文件夹中，如下所示：https://beerswift.app/.well-known/。

继续查看 iOS 和 Android 的配置细节。

## iOS 配置

iOS 配置包括创建站点关联文件以及配置原生应用以识别应用域名。
...
接下来，将文件上传到您的网站（托管在 HTTPS 上），然后使用 Apple 的[工具](https://search.developer.apple.com/appsearch-validation-tool/)验证其配置是否正确。URL 应遵循此格式：https://beerswift.app/.well-known/apple-app-site-association

### 添加 Associated Domain
...
Android 配置包括创建站点关联文件以及使用 intent filter 配置原生应用以识别应用链接。
...
最后一步是配置 Android 应用以识别传入的链接。为此，请在 `AndroidManifest.xml` 的 `<activity>` 元素内[添加新的 Intent Filter](https://developer.android.com/training/app-links/deep-linking#adding-filters)：
...
将关联文件放在 `src/.well-known` 下。接下来，配置构建过程以原样部署这些文件（确保 Apple/Google 可以正确读取它们）。打开 `angular.json`，在 `architect => assets` 下，添加一个新条目到数组中：
...
将关联文件放在 `static/.well-known` 下。不需要额外步骤；只需构建然后部署网站。

### React

将关联文件放在 `public/.well-known` 下。不需要额外步骤；只需构建然后部署网站。

### WordPress

WordPress 的说明请[参见此处](https://devdactic.com/universal-links-ionic/)。

## 验证

要验证网站和原生应用的配置是否正确，网站需要托管站点关联文件，但应用不需要在应用商店中。

将设备连接到您的计算机，构建并部署原生应用，然后通过点击网站链接进行测试。如果原生应用打开，则表示所有步骤都已正确实施。
