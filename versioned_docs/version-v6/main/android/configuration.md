...
本文涵盖了你需要对应用进行的基本修改。阅读 [Android 清单文档](https://developer.android.com/guide/topics/manifest/manifest-intro.html) 以了解更多内容。

## 修改包 ID

要更改应用的包 ID（Android 中也称为**应用 ID**），请编辑 `android/app/build.gradle` 顶部的 `applicationId`：
...
将活动名称更改为匹配的名称可能很有意义，特别是当你的应用只有一个活动时：
...
要启用通过 Android 应用链接的深度链接，请遵循官方 Android 指南 [添加 Android 应用链接](https://developer.android.com/studio/write/app-link-indexing)。Android Studio 提供了一个便捷的向导来配置应用链接。

一旦配置完成，[App API 中的 `getLaunchUrl()` 方法](/apis/app.md#getlaunchurl) 将提供应用启动时使用的任何 URL，并且当应用收到新的应用链接深度链接时，[`'appUrlOpen'` 事件](/apis/app.md#addlistenerpause-) 将触发。
...
通常情况下，你选择使用的插件会要求你设置权限。请在此文件中添加它。