...
可能并非所有功能都有现成的 [Capacitor 插件](/plugins.mdx)——但这没关系！你完全可以直接在应用中编写 WebView 可以访问的原生代码。
...
首先，创建一个 `EchoPlugin.java` 文件。你可以通过 [打开 Android Studio](/main/android/index.md#opening-the-android-project)，展开 **app** 模块和 **java** 文件夹，右键点击应用的 Java 包，从上下文菜单中选择 **New** -> **Java Class** 来创建文件。
...
在应用的 `MainActivity.java` 中，使用 `registerPlugin()` 或 `registerPlugins()` 来注册你的自定义插件。
