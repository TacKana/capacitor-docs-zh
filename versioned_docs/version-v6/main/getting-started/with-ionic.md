...
Capacitor 不要求使用 Ionic Framework 来构建应用。但是，开发者可能会发现 [Ionic UI 组件的大型集合](https://ionicframework.com/docs/components) 有助于构建高质量的应用。

Capacitor 可以通过使用 [Ionic CLI](https://ionicframework.com/docs/cli) 直接快速安装到任何新的或现有的 Ionic 应用中。

## 在新的 Ionic 项目中安装 Capacitor
对于新的 Ionic 项目，Capacitor 默认已安装在新 Ionic 应用中！您只需启动一个新项目即可。要创建新的 Ionic 项目，请运行以下命令：

```bash
ionic start
```

如果您想要一个构建第一个基于 Capacitor 的 Ionic 应用的教程，请查看 Ionic Framework 团队的[此教程](https://ionicframework.com/docs/intro/next)。

## 向现有的 Ionic 项目安装 Capacitor
如果您有一个未启用 Capacitor 的现有 Ionic 项目，可以通过运行以下命令来启用 Capacitor。
...
为了获得最佳用户体验，即使您不在应用中导入它们，也应确保安装这些插件。要安装这些插件，请在项目根目录中运行以下命令：
...
安装 Capacitor 及其插件后，您可以将移动平台添加到应用中：
...
这将在项目根目录中为原生平台创建一个新目录。此目录是一个原生项目，应被视为源工件。了解更多关于[原生项目管理](/main/cordova/index.md#native-project-management)的信息。
...
Ionic CLI 有一些高级命令，它们封装了 Capacitor CLI 以方便使用。请参阅下面的每个命令的文档。每个命令后也可以使用 `--help` 标志获取帮助输出。
...
有关 Ionic CLI 的更多信息以及如何将其与 Capacitor 一起使用，您可以查看[此处的文档](https://ionicframework.com/docs/cli)。
