...
在 Capacitor 6 中，插件类不再自动注册。对于通过 npm 安装的插件，CLI 将生成一个插件类列表，以编程方式注册它们。但是，遵循[自定义代码指南](../ios/custom-code.md)创建未通过 npm 分发的本地插件的用户，将需要创建[自定义视图控制器并注册他们的插件](../ios/custom-code.md#注册插件)。
