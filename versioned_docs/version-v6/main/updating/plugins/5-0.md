---
title: 插件升级至 5.0 指南
description: 将插件中的 Capacitor 从早期版本升级至 v5 的指南
...
* `pluginId` 重命名为 `identifier`，以避免与 `CAPPlugin.pluginId` 冲突，同时移除了 `getMethod(_:)` 要求，并将其放入内部的扩展方法中。
* `pluginMethods` 的声明也进行了更新，使其内容类型更加明确（从 `Any` 更改为 `CAPPluginMethod`）。
