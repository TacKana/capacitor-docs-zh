...
根据您的偏好以及是在网页浏览器中测试还是在真实或模拟的移动设备上测试，有几种不同的方式来构建或运行您的应用。

:::tip
您可以按 `⌥` + R 来运行您的应用（在 Windows 上是 `ALT` + `R`）。
:::

## 在 Web 上测试
点击 `运行` > `Web` 以在默认的网页浏览器中构建并启动应用。

:::note
您可以在 VS Code 编辑器内预览您的应用，只需在 `Web` 旁边的 `...` 上点击，并选择 `在编辑器中打开应用`。
:::
...
如果您需要进行需要 [安全上下文](https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts) 的 Web API 调用，例如 [Geolocation](https://developer.mozilla.org/en-US/docs/Web/API/Navigator/geolocation)，则必须通过 `HTTPS` 提供您的应用。
