...
该扩展将根据 Ionic 工程师发现的常见问题的经验提出建议。建议以灯泡图标显示，可以执行或忽略。

## 常见建议

以下情况会显示建议：
- 当包被其作者在 GitHub 中弃用或归档时
...
- 如果某个不兼容的插件可以被等效的 Capacitor 插件替代时
- 当某个插件的功能已内置于 Capacitor 中时
- 当某个依赖已被替换时（例如 `ionic-native` -> `awesome-cordova-plugins`）
- 如果某个插件或依赖存在需要解决的安全漏洞时
- 当某个插件不是必需的时候（例如 `cordova-plugin-add-swift-support`）

:::note
并非所有场景都涵盖，因此如果您需要对项目应用某个修复，并且认为这将对其他开发者有益，请[提交建议](https://github.com/ionic-team/vscode-ionic/issues)。
:::
