...
description: Capacitor Cookies API 通过修补 `document.cookie` 以使用原生库的方式提供原生 Cookie 支持。
...
Capacitor Cookies API 通过修补 `document.cookie` 以使用原生库的方式提供原生 Cookie 支持。它还提供了在特定 URL 修改 Cookie 的方法。此插件随 `@capacitor/core` 一起提供。
...
| 属性          | 类型                 | 描述                                                               | 默认值            |
| ------------- | -------------------- | ------------------------------------------------------------------ | ------------------ |
| **`enabled`** | <code>boolean</code> | 启用修补 `document.cookie` 以使用原生库的功能。 | <code>false</code> |
...
从 iOS 14 开始，默认情况下不能使用第三方 Cookie。请将以下内容添加到你的 Info.plist 文件中，以获得更好的 iOS Cookie 支持。最多可以添加 10 个域名。
