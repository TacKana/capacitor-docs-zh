...
如果你使用的是 <b>Xcode 15 Beta 4 或更高版本</b>，则需要从 node_modules 中添加 Capacitor Watch Swift Package：
...
然后打开手表应用的"Main"文件，该文件应为 `watchappApp.swift`。在 `@main` 语句上方添加 `import WatchConnectivity` 这一行。然后将写着 `ContentView()` 的那一行替换为：
