---
title: Keyboard - Capacitor 键盘插件 API
...
| **`style`**              | <code><a href="#keyboardstyle">KeyboardStyle</a></code>   | 如果你的应用不支持深色/浅色主题切换，则覆盖键盘样式。如果未设置，键盘样式将取决于设备外观。仅在 iOS 上可用。                                                                                                                                                                                             |                     | 1.0.0 |
| **`resizeOnFullScreen`** | <code>boolean</code>                                      | 有一个 Android 错误，阻止键盘在应用全屏时调整 WebView 大小（例如，如果使用 StatusBar 插件来重叠状态栏）。如果设置为 true，此设置会添加一个解决方案，使 WebView 即使在应用全屏时也能调整大小。仅 Android 可用。                                                                                                 |                     | 1.1.0 |
...
设置键盘上的辅助工具栏是否可见。我们建议在短表单（登录、注册等）上禁用
辅助工具栏，以提供更简洁的用户界面。
...
| 成员      | 值                 | 描述                                                                                                          | 自从 |
| ------------ | --------------------- | -------------------------------------------------------------------------------------------------------------------- | ----- |
| **`Body`**   | <code>'body'</code>   | 只有 `body` HTML 元素会被调整大小。相对单位不受影响，因为视口不会改变。 | 1.0.0 |
| **`Ionic`**  | <code>'ionic'</code>  | 只有 `ion-app` HTML 元素会被调整大小。仅用于 Ionic Framework 应用。                               | 1.0.0 |
| **`Native`** | <code>'native'</code> | 整个原生 Web View 会在键盘显示/隐藏时调整大小。这会影响 `vh` 相对单位。        | 1.0.0 |
