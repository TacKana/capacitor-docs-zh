...
首先，按照插件指南的[入门](/plugins/creating-plugins/overview.md#plugin-generator)部分中所示生成一个插件。
...
您需要开发一个或多个别名来抽象和分组您的插件所需的权限。这些别名用于传达权限状态。默认情况下，别名可以处于以下状态之一：

- `granted`：此别名中的所有权限已被最终用户授予（或不需要提示）。
- `denied`：此别名中的一个或多个权限已被最终用户拒绝。
- `prompt`：应该提示最终用户授予权限，因为既未授予也未拒绝。
- `prompt-with-rationale`：最终用户之前拒绝过权限，但尚未阻止提示。
...
此方法应返回插件中权限的当前状态。这些信息可能直接在特定的 Web API 上可用，或通过 [Permissions API](https://developer.mozilla.org/en-US/docs/Web/API/Permissions_API) 获得。
