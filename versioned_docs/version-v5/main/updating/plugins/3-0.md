...
原`@NativePlugin`注解已弃用。推荐使用新的`@CapacitorPlugin`注解以支持[新版权限API](#采用新版权限API)。
...
Android解析带alpha通道的十六进制颜色字符串为_ARGB_格式，而iOS和Web端解析为_RGBA_格式。如需跨平台共享带透明通道的颜色值，请使用新的`WebColor`工具类。`WebColor.parseColor()`功能类似原生Android的`Color.parseColor()`，但会按照RGBA格式解析字符串。
