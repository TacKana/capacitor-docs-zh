---
title: 将 Capacitor 与 Ionic Framework 一起使用
...
Capacitor 可以通过使用 [Ionic CLI](https://ionicframework.com/docs/cli) 快速直接安装到任何新的或现有的 Ionic 应用中。

## 在新的 Ionic 项目中安装 Capacitor
对于新的 Ionic 项目，默认情况下 Capacitor 已经安装在新创建的 Ionic 应用中！您只需启动一个新项目即可。要创建新的 Ionic 项目，请运行以下命令：

```bash
ionic start
```

如果您想要一个构建第一个基于 Capacitor 的 Ionic 应用的教程，请查看 Ionic Framework 团队的[这个教程](https://ionicframework.com/docs/intro/next)。

## 将 Capacitor 安装到现有的 Ionic 项目
...
为了获得最佳用户体验，即使您不在应用中导入这些插件，也应确保安装它们。要安装这些插件，请在项目根目录运行以下命令：
...
这将在项目根目录中为原生平台创建一个新目录。此目录是一个原生项目，应被视为源资产。了解更多关于[原生项目管理](/main/cordova/index.md#native-project-management)的信息。
...
Ionic CLI 有各种高级命令，为方便起见封装了 Capacitor CLI。请参阅下面每个命令的文档。在每个命令后使用 `--help` 标志也可以查看帮助输出。
...
有关 Ionic CLI 及其如何与 Capacitor 一起使用的更多信息，请参见[此处](https://ionicframework.com/docs/cli)的文档。
