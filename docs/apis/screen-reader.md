...
description: Screen Reader API 提供对 TalkBack/VoiceOver 等功能的访问，并为视觉辅助提供简单的文本转语音能力。
custom_edit_url: https://github.com/ionic-team/capacitor-plugins/blob/next/screen-reader/README.md
...
Screen Reader API 提供对 TalkBack/VoiceOver 等功能的访问，并为视觉辅助提供简单的文本转语音能力。
...
ScreenReader.addListener('stateChange', ({ value }) => {
  console.log(`屏幕阅读器现在为 ${value ? '开启' : '关闭'}`);
});
...
更多文本转语音功能，请参阅 [Capacitor Community 文本转语音插件](https://github.com/capacitor-community/text-to-speech)。
...
添加一个监听器，用于监听屏幕阅读器开启或关闭的事件。

此事件此前名为 `'accessibilityScreenReaderStateChange'`。
