...
| **`recursive`**  | <code>boolean</code>                            | 是否同时创建所有缺失的父级目录。                                | <code>false</code>   | 1.0.0    |

#### RmdirOptions（删除目录选项） {#rmdiroptions}
...
#### ReaddirResult（读取目录结果） {#readdirresult}
...
#### ReaddirOptions（读取目录选项） {#readdiroptions}
...
#### GetUriResult（获取 URI 结果） {#geturiresult}
...
#### GetUriOptions（获取 URI 选项） {#geturioptions}
...
#### DownloadFileResult（下载文件结果） {#downloadfileresult}

| 属性            | 类型                  | 描述                                                               | 始于 |
| --------------- | --------------------- | ------------------------------------------------------------------ | ---- |
| **`path`**      | <code>string</code>   | 文件下载到的路径                                                   | 5.1.0 |
| **`blob`**      | <code>Blob</code>     | 已下载文件的 Blob 数据。此属性仅在 Web 端可用                      | 5.1.0 |
