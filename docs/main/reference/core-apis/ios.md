...
Capacitor iOS 是为 iOS 平台上的 Capacitor 应用提供支持的原生运行时环境。
...
在 JavaScript 的 [`EventTarget`](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget)（例如 `window` 或 `document`）上触发事件。如果可能，建议优先使用 [Plugin Events](/plugins/creating-plugins/ios-guide.md#plugin-events)。
...
将来自 Web 视图的 URL 转换为适用于原生 iOS 环境的文件 URL。

Web 视图可能处理多种不同类型的 URL：

- `res://`（Web 资源的快捷方案）
- `file://`（指向本地设备文件的完整 URL）
...
将适用于原生 iOS 环境的文件 URL 转换为可在 Web 视图中加载的 URL。

---

## 数据传递

关于如何在环境间处理传递数据的说明，请[参见此处](/main/reference/core-apis/data-types.md#ios)。
