...
可能并非所有功能都有[对应的 Capacitor 插件](/plugins.mdx)——这没关系！你完全可以在应用中直接编写 WebView 可访问的原生代码。

## WebView 可访问的原生代码

在 JavaScript 和原生代码之间进行通信的最简单方式，是构建一个应用本地的自定义 Capacitor 插件。

### `EchoPlugin.java`

首先，通过[打开 Android Studio](/main/android/index.md#opening-the-android-project)，展开 **app** 模块和 **java** 文件夹，右键点击应用的 Java 包，从上下文菜单中选择 **New** -> **Java Class**，创建 `EchoPlugin.java` 文件。
...
在 JavaScript 中，我们使用 `@capacitor/core` 中的 `registerPlugin()` 来创建一个链接到 Java 插件的对象。
...
我们可以通过定义接口并在调用 `registerPlugin()` 时使用该接口，来为链接的对象定义类型。
...
`registerPlugin()` 的泛型参数定义了链接对象的结构。如果需要，你可以使用 `registerPlugin<any>('Echo')` 来忽略类型。没关系的。❤️

### 使用插件

使用导出的 `Echo` 对象来调用你的插件方法。以下代码片段将调用 Android 上的 Java 代码并打印结果：
