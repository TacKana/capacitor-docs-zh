...
我们必须在 Android 端和 Web 端都注册自定义插件，以便 Capacitor 能够在 Java 和 JavaScript 之间建立桥梁。
...
> `registerPlugin()` 的第一个参数是插件名称，它必须与 `EchoPlugin.java` 中 `@CapacitorPlugin` 注解的 `name` 属性匹配。
...
`registerPlugin()` 的泛型参数定义了链接对象的结构。如果需要，您可以使用 `registerPlugin<any>('Echo')` 来忽略类型。我们不会对此进行评判。❤️
