...
iOS 推送通知 API 使用 Swift Package Manager 或 CocoaPods 进行依赖管理。我们需要告知它们使用 Firebase。

#### 使用 Swift Package Manager (SPM)

要通过 SPM 添加 SDK，您需要修改 `ios/App/App.xcodeproj` 文件。

首先，通过运行 `npx cap open ios` 或在 Finder 中双击该文件，在 Xcode 中打开 `ios/App/App.xcodeproj`。

在左侧选择您的 App，在右侧选择 "Package Dependencies"，如下图所示。

![SPM-FB-Step1](../../../static/img/spm/firebase/firebase-spm-step1.png)

然后，选择加号图标以添加新的包，应显示类似下图的界面。
...
现在滚动并选择将 Firebase Messaging 添加到 App target。

![SPM-FB-Step3](../../../static/img/spm/firebase/firebase-spm-step3.png)

点击 "Add Package"，并等待处理完成。完成后，您应该看到类似下图的界面。
