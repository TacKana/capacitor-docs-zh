...
大多数模拟库通过包装 JavaScript 代理对象来创建模拟，以便检查对该对象方法的调用并控制方法的返回值。然而，Capacitor 插件在 JavaScript 层是以代理形式实现的。不支持也不允许创建代理的代理。手动模拟可以用来解决这个问题。

## 手动模拟

手动模拟允许用户轻松地模拟整个 JavaScript 模块的功能。因此，当测试执行 `import { Storage } from '@capacitor/storage'` 时，测试将加载类似以下的内容，而不是真正的 `Storage` JavaScript 代理对象：
...
Jest 测试框架内置了<a href="https://jestjs.io/docs/manual-mocks" _target="blank">手动模拟</a>功能。在项目根目录创建一个 `__mocks__/@capacitor` 文件夹，Jest 将自动从该文件夹加载文件，而不是从 `node_modules` 加载。
...
首先，与 Jest 示例类似，在项目根级别创建相同的目录结构。

Angular 项目（最常使用 Jasmine 作为测试框架的场景）包含一个 `tsconfig.spec.json` 文件，在执行单元测试时会扩展 `tsconfig.json` 的基本配置。修改此文件以扩展你在基础级别可能设置的任何 `paths` 映射。
...
然后更新你的 `tsconfig.spec.json` 文件，包含这些路径以及你希望用于单元测试的任何其他路径：
...
通过手动模拟，现在可以以所有常规方式编写测试来模拟和监视方法调用。
