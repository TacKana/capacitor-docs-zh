...
为了使 Apple 和 Google 能够自动填充和保存凭据，必须在你的网站和应用之间配置双向关联。在本指南中，我们将遵循与[深度链接](deep-links#create-site-association-file-1)相同的步骤，但会增加 [Capacitor 配置](#set-capacitor-server-hostname)和 `autocomplete` 属性的使用步骤。
...
## Web 配置
如果你的目标是 Web 平台，请按照[深度链接指南](deep-links#website-configuration)进行配置。
