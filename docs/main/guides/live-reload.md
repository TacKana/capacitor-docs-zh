...
`server` 条目会在 `capacitor.config.json` 中自动创建，并在命令终止后被移除。有关 `ionic cap run` 命令的完整详细信息，[请参见此处](https://ionicframework.com/docs/cli/commands/capacitor-run)。
...
- 在 macOS 上，运行 `ifconfig`。IP 地址列在 `en0` 条目下 `inet` 之后。或者，打开 系统偏好设置 -> 网络 -> （选择活跃网络），然后在状态下方找到列出的 IP。
