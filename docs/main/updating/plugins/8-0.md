---
title: 将插件升级到 8.0
description: 在您的插件中将 Capacitor 从 v7 升级到 v8 的指南
slug: /updating/plugins/8-0
---

# 在您的插件中将 Capacitor 升级到 8.0
...
在您的 `build.gradle` 文件中，更新以下包的版本：

```diff
ext {
+   // 注意：以下部分依赖是可选的 - 只添加/更新您的插件实际使用的依赖。
...
我们推荐使用 Java 21 以获得与 Android Gradle Plugin 8.13.0 和 Android SDK 36 的最佳兼容性，不过 Java 17+ 也受支持。
...
如果您的插件使用 kotlin，请将默认的 `kotlin_version` 更新到 `2.2.20`。这是从 Kotlin 1.x 到 2.x 的重大版本升级，包含了破坏性变更。
...
Kotlin 2.2.0 包含了来自 Kotlin 1.x 的几个破坏性变更。以下是需要注意的关键变更：

- **`kotlinOptions{}` 代码块**：Gradle 中的 `kotlinOptions{}` 代码块已被弃用并会引发错误。请将其替换为 `compilerOptions{}` 代码块（如下所示）。
- **移除 `kotlin-android-extensions` 插件**：`kotlin-android-extensions` 插件已被移除。对于 `Parcelable` 实现请使用 `kotlin-parcelize` 插件，对于合成视图请使用 Android Jetpack 的视图绑定。

有关破坏性变更的完整列表和迁移指南，请参阅 [Kotlin 2.2.0 发布说明](https://kotlinlang.org/docs/whatsnew22.html)。

#### 将 kotlinOptions 更新为 compilerOptions

Kotlin 2.0+ 需要改用新的 `compilerOptions` API 而不是 `kotlinOptions`。`android {}` 内部的 `kotlinOptions` 代码块已被弃用。
...
注意：如果您使用的是 Java 21（推荐），请使用 `JvmTarget.JVM_21`，或者使其与您的 Java 版本匹配。Java 17+ 受支持，但推荐使用 Java 21 以获得最佳兼容性。
...
针对您的 Xcode 项目执行以下操作：在项目编辑器中选择 **Project** 并打开 **Build Settings** 标签页。在 **Deployment** 部分，将 **iOS Deployment Target** 更改为 **iOS 15.0**。对任何应用 **Targets** 重复相同的步骤。

然后，打开 `ios/Podfile` 并将 iOS 版本更新至 15.0：
