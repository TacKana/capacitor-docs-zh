---
title: 插件升级到 8.0
description: 在插件中将 Capacitor 从 v7 升级到 v8 的指南
slug: /updating/plugins/8-0
---

# 在插件中将 Capacitor 升级到 8.0

## 使用 @capacitor/plugin-migration-v7-to-v8

在插件文件夹中，运行 `npx @capacitor/plugin-migration-v7-to-v8@latest`，它将自动执行所有文件更改。
...
在你的 `build.gradle` 文件中，更新以下包版本：

```diff
ext {
+   // 注意：以下一些依赖项是可选的 - 只添加/更新你的插件实际使用的依赖项。
+   // 如果你使用了以下任何依赖项，请将它们更新为下方显示的版本。
...
### 更新 targetSDK / compileSDK 到 36 以及 minSDK 到 24
...
Gradle 8 弃用了属性的空格赋值语法。将所有属性赋值更新为使用 `=` 运算符。有关更多详细信息，请参阅 [Gradle 升级指南](https://docs.gradle.org/current/userguide/upgrading_version_8.html#groovy_space_assignment_syntax)。
...
注意：属性赋值需要使用 `=` 运算符。方法调用（如 `mavenCentral()`）保持不变，不使用 `=` 运算符。

### 更新到 Java 21（推荐）

我们建议使用 Java 21 以获得与 Android Gradle Plugin 8.13.0 和 Android SDK 36 的最佳兼容性，不过 Java 17+ 也受支持。
...
如果你的插件使用 kotlin，请将默认的 `kotlin_version` 更新为 `2.2.20`。这是从 Kotlin 1.x 的主版本升级，包含破坏性变更。
...
Kotlin 2.2.0 包含若干来自 Kotlin 1.x 的破坏性变更。以下是需要注意的关键变更：

- **`kotlinOptions{}` 代码块**：Gradle 中的 `kotlinOptions{}` 代码块已弃用并会引发错误。请将其替换为 `compilerOptions{}` 代码块（如下所示）。
...
有关破坏性变更和迁移指南的完整列表，请参阅 [Kotlin 2.2.0 发布说明](https://kotlinlang.org/docs/whatsnew22.html)。

#### 将 kotlinOptions 更新为 compilerOptions

Kotlin 2.0+ 要求使用新的 `compilerOptions` API 替代 `kotlinOptions`。`android {}` 内部的 `kotlinOptions` 代码块已弃用。
...
注意：如果你使用的是 Java 21（推荐），请使用 `JvmTarget.JVM_21`，或者根据你的 Java 版本进行匹配。Java 17+ 受支持，但建议使用 Java 21 以获得最佳兼容性。
...
在 Xcode 项目中执行以下操作：在项目编辑器中选择 **Project**，然后打开 **Build Settings** 选项卡。在 **Deployment** 部分下，将 **iOS Deployment Target** 更改为 **iOS 15.0**。对任何应用 **Targets** 重复相同的步骤。

然后，打开 `ios/Podfile` 并将 iOS 版本更新为 15.0：
...
在 SPM 兼容的插件中，更新 `Package.swift` 文件以使用版本 `8.0.0`
