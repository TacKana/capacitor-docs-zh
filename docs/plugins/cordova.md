...
> 如果 Cordova 插件包含 [`@awesome-cordova-plugins`](https://ionicframework.com/docs/native) 封装，你也可以安装它以获得 TypeScript 支持：
>
> ```bash
...
使用常规的包管理器来更新插件。然后，将更新后的插件同步到原生项目：
...
查看项目中已安装的 Capacitor 和 Cordova 插件（及其精确版本号）：
...
Capacitor 不支持 Cordova 安装变量、自动配置或钩子，因为我们的理念是让你控制原生项目源代码（这意味着钩子之类的东西是不必要的）。如果你的插件需要设置变量或配置，你需要手动应用这些配置设置，在插件的 `plugin.xml` 和 iOS 及 Android 上所需的设置之间进行映射。

请查阅 [iOS](/main/ios/configuration.md) 和 [Android](/main/android/configuration.md) 配置指南，了解如何配置每个平台。

### 已知不兼容的插件

如果某个插件已知会导致冲突或构建问题，则在运行 `npx cap sync` 时会被跳过。
...
- [`cordova-plugin-console`](https://github.com/apache/cordova-plugin-console)（不需要，Capacitor 自带）
...
- [`cordova-plugin-ionic-keyboard`](https://github.com/ionic-team/cordova-plugin-ionic-keyboard)（不需要，Capacitor 自带）
...
- [`cordova-plugin-splashscreen`](https://github.com/apache/cordova-plugin-splashscreen)（不需要，Capacitor 自带）
- [`cordova-plugin-statusbar`](https://github.com/apache/cordova-plugin-statusbar)（不需要，Capacitor 自带）
