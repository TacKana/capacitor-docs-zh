...
插件开发已接近完成，现在只剩下 Android 平台的实现了！
...
从上下文菜单中选择 **New -> Package**，创建一个名为 **plugins** 的子包。右键点击 **plugins** 包，重复上述过程创建一个名为 **ScreenOrientation** 的子包。

接着，右键点击 **ScreenOrientation** 包，通过选择 **New -> Java File** 从上下文菜单添加一个新的 Java 文件。将此文件命名为 `ScreenOrientationPlugin.java`。重复该过程创建另一个名为 `ScreenOrientation.java` 的新文件。
...
在项目的 MainActivity 中注册插件类，以连接 Java 和 JavaScript。打开 `MainActivity.java` 并添加一个 `onCreate()` 方法，用于注册插件：
...
与 iOS 类似，我们首先处理获取当前屏幕方向的问题。打开 `ScreenOrientation.java` 来设置类并编写获取当前方向的方法：
...
> **注意：** 日志的确切值会因情况而异。在此示例中，`89582874` 是从插件调用的方法被分配的任意 ID。
...
为了解决这些挑战，我们需要插件跟踪之前的 `newConfig.orientation` 值，以便与后续的配置更改进行比较。
...
在 Android Studio 中，在设备或模拟器上运行应用。按下“旋转我的设备”按钮将使屏幕方向旋转到横屏模式，如果你进一步旋转设备，会看到屏幕方向已被锁定。按下“确认签名”将解锁屏幕方向。

> **注意：** 在测试插件之前，请确保设备设置中的**自动旋转**选项已**开启**；否则插件将无法正常工作。

恭喜，你已经构建了一个适用于 Web、iOS 和 Android 的 Capacitor 插件！👏 👏 👏

目前，`ScreenOrientation` 插件是一个本地插件；只有这个应用程序可以使用它。这完全没问题！很多时候你只需要在特定应用中使用插件。但是，如果你想在多个应用中复用插件，我们将在最后一步中看到如何操作：打包插件。