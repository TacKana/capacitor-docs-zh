...
在下面的示例中，添加了 `openMap()` 方法，该方法接收 `latitude` 和 `longitude` 参数。良好的做法是为方法参数定义接口，以便在应用中导入和使用。
...
此示例包含插件中最常见的方法类型，但所有支持类型的详细信息[可在此处找到](/plugins/creating-plugins/method-types.md)。

## 本地测试

要在开发过程中本地测试插件，使用 `npm install` 将插件文件夹链接到你的应用中，并指定插件路径。

```bash
npm install ../path/to/echo
```

项目的 `package.json` 文件现在将在依赖项列表中显示插件包链接：

```json
"echo": "file:../path/to/echo",
```

最后，运行 `npx cap sync` 让原生项目识别你的插件。如果检测正确，它将打印类似以下内容：
...
要从应用中取消链接本地插件，使用 `npm uninstall` 并指定插件包名。
...
插件模板附带 [`@capacitor/docgen`](https://github.com/ionic-team/capacitor-docgen)，它将生成的文档写入 `README.md`。文档在 `npm run build` 期间生成。你也可以手动运行：
...
现在你的包可以在任何 Capacitor 应用中使用 `npm install echo` 进行安装。