...
此示例展示了插件中最常见的方法类型，但有关所有支持类型的详细信息，[请参见此处](/plugins/creating-plugins/method-types.md)。

## 本地测试

要在开发过程中本地测试插件，请使用 `npm install` 并将路径指向您的插件文件夹，将插件链接到您的应用。
...
最后，运行 `npx cap sync` 使原生项目识别您的插件。如果正确检测到，它将打印类似以下信息：
...
要从您的应用中取消本地插件的链接，请使用 `npm uninstall` 并指定插件的包名。
...
插件模板附带了 [`@capacitor/docgen`](https://github.com/ionic-team/capacitor-docgen)，它会将生成的文档写入 `README.md`。文档在 `npm run build` 期间生成。您也可以手动运行：
...
这将构建插件的 JS 部分，并将剩余的插件文件发布到 npm。

现在，您的包可以使用 `npm install echo` 安装到任何 Capacitor 应用中。
